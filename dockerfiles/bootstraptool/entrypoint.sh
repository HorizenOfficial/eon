...
if [ "$#" -eq 0 ] || [ "${1}" = "/usr/bin/true" ]; then
  set -- java -jar /tools/"${SC_JAR_NAME}-${SC_VERSION}.jar" help
elif [[ "${1}" == +(help|generatekey|generateVrfKey|generateCertProofInfo|generateCertWithKeyRotationProofInfo|generateCswProofInfo|genesisinfo|generateAccountKey|generateCertificateSignerKey) ]]; then
  set -- java -jar /tools/"${SC_JAR_NAME}-${SC_VERSION}.jar" "$@"
fi
